...
@interface JWKContactModel ()
{
    BOOL toBeUpdated;
}
@end

@implementation JWKContactModel
...
-(void)setEmailAddresses:(NSArray<JWKContactLabelStringModel *> *)emailAddresses {
...
-(void)setPostalAddresses:(NSArray<JWKContactLabelDictionaryModel *> *)postalAddresses {
    NeedsToUpdateArrayValue(postalAddresses, kABPersonAddressProperty);
}

-(void)setDates:(NSArray<JWKContactLabelDateModel *> *)dates {
...
-(void)setPhoneNumbers:(NSArray<JWKContactLabelStringModel *> *)phoneNumbers {
    NeedsToUpdateArrayValue(phoneNumbers, kABPersonPhoneProperty);
}

-(void)setInstantMessageAddresses:(NSArray<JWKContactLabelDictionaryModel *> *)instantMessageAddresses {
    NeedsToUpdateArrayValue(instantMessageAddresses, kABPersonInstantMessageProperty);
}

-(void)setUrlAddresses:(NSArray<JWKContactLabelStringModel *> *)urlAddresses {
    NeedsToUpdateArrayValue(urlAddresses, kABPersonURLProperty);
}

-(void)setContactRelations:(NSArray<JWKContactLabelStringModel *> *)contactRelations {
    NeedsToUpdateArrayValue(contactRelations, kABPersonRelatedNamesProperty);
}

-(void)setSocialProfiles:(NSArray<JWKContactLabelStringModel *> *)socialProfiles {
...
static inline void UpdateArrayValue(ABRecordRef record,ABPropertyID property,NSArray <JWKContactLabelModel *> *values,BOOL * update) {
    if (record) {
        *update = YES;
        if (values.count) {
...
static inline void SetArrayWithProperty(ABRecordRef record,ABPropertyID property,NSArray <JWKContactLabelModel *> *values) {
    if (!values.count) {
        return;
    }
    ABMultiValueRef multi = ABMultiValueCreateMutable(kABStringPropertyType);
    for (JWKContactLabelModel * model in values) {
...
-(NSArray *)getLabelValueWithABRecord:(ABRecordRef)ABRecord property:(ABPropertyID)property {
    ...
    for (int i = 0; i < count; i++) {
        JWKContactLabelModel * model = [JWKContactLabelModel new];
...
@implementation JWKContactLabelModel

@end

@implementation JWKContactLabelStringModel
@dynamic labelValue;
@end

@implementation JWKContactLabelDictionaryModel
@dynamic labelValue;
@end

@implementation JWKContactLabelDateModel
@dynamic labelValue;
@end
